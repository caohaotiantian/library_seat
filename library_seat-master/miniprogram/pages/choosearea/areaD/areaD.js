// pages/choosearea/areaD/areaD.js
...
function count_down(that) {
  ...
  if (micro_second <= 0) {
    ...
    that.setData({
      deskinfo: information.HalldeskCondition,
...
Page({
  ...
  choose: function (event) { //用户点击座位
    if (information.userCondition.credit < 60) { //信誉小于60，发出警报
      ...
      if (evinfo.class == "chair cx0") { //座位空闲
        information.userCondition.reserve = ["走廊", parseInt(evinfo.pos.split('x')[1]), evinfo.id];
  ...
  determine: function (event) { //用户点击“确认预约”
    ...
    var that = this;
    if (information.HalldeskCondition[x[1] - 1][posit] == "chair cx0") {
      information.HalldeskCondition[x[1] - 1][posit] = "chair cx1"; //改为不可选
      information.userCondition.condition = 2; //用户状态改变
      information.Hallcount++; //B区座位被占用数改变
      wx.showModal({
        title: '完成',
  ...
  onLoad: function (options) {
    ...
    if (information.userCondition.condition != 0 && information.userCondition.reserve[0] == '走廊') {
      var x = information.userCondition.reserve[1] - 1;
      var y = "c" + information.userCondition.reserve[2];
      this.data.deskinfo[x][y] = 'chair mine';
      this.setData({
